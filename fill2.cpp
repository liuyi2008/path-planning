...
struct point
{
	float x;
	float y;
}p;            //polypoint[7] = { {250,50},{500,150},{550,400},{250,450},{100,350},{100,100},{120,30} };//多边形顶点
...
void PolyScan()
{
		...
		for (int j = 0; j < POINTNUM; j++)
			if (polypoint[j].y == i)
			{
				...
				if (polypoint[(j - 1 + POINTNUM) % POINTNUM].y > polypoint[j].y)
				{
					...
					p->next = pNET[i]->next; //为啥不是p->next = NULL;
	...
	for (i = MinY; i <= MaxY; i += Width)
	{
		...
		while (p && p->next)
		{
			printf("扫描线i=%d,交点一X=%f,dx=%f,ymax=%f      ", i, p->x, p->dx, p->ymax);
			printf("扫描线i=%d,交点二X=%f,dx=%f,ymax=%f\n", i, p->next->x, p->next->dx, p->next->ymax);
...
void PolyScantest()
{
	...
	for (i = MinY; i <= MaxY; i += Width)
	{
		if (i < p->ymax && i < q->ymax)     //情况1，2
		{
			...
			{
				float j = p->x;
				float h = p->next->x - 10;//这里左平移
				glBegin(GL_LINES);
				glVertex2f(j, i); glVertex2f(h, i);
				glVertex2f(j, i - 2); glVertex2f(h, i - 2); //这里下平移
				glEnd();
				p->x = p->x + p->dx;
				q->x = q->x + q->dx;
				continue;
		...
		else if (i >= p->ymax && i < q->ymax)  //情况3，3.1
		{
			for (int ii = i; ii > i - Width; ii--)
			{
				...
				{
					p = pNET[ii]->next;
					p->x = p->x + (i - ii) / 10 * p->dx;          //i-ii是间隙的距离
					continue;
				}
			}
...
void myDisplay()
{
	...
	for (int i = 0; i < polypoint.size(); i++)
	{
		glVertex2i(polypoint[i].x, polypoint[i].y);  //wh-y  反转
